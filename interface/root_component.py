# 4 Main components in the dashboard:

# - Strategies - top right
# - Logging - bottom right
# - Trades - bottom left
...
# where each aspects of the dashboard go
class Root(tk.Tk):
    def __init__(self, binance: BinanceClient, bitmex: BitmexClient):
        ...
        self._watchlist_frame = Watchlist(self.binance.contracts, self.bitmex.contracts, self._left_frame, bg='blue')
        self._watchlist_frame.pack(side=tk.TOP, padx=10)

        self.logging_frame = Logging(self._left_frame, bg='red')
        self.logging_frame.pack(side=tk.TOP, pady=15)  # Space a bit the components with vertical padding

        self._strategy_frame = StrategyEditor(self, self.binance, self.bitmex, self._right_frame, bg='yellow')
        self._strategy_frame.pack(side=tk.TOP, pady=15)

        self._trades_frame = TradesWatch(self._right_frame, bg='light grey')
