...
class TechnicalStrategy(Strategy):
    ...
    def _rsi(self): # relative strenght index
        close_list = []
    ...
    def _macd(self) -> Tuple[float, float]: #moving average convergence divergence
        # EMA Exponential Moving Average
        # MACD calculation steps 1. Fast EMA 2. SLow EMA, 3 Fast - slow EMA 4 EMA on the result of 3
        close_list = []
        for candle in self.candles:
            close_list.append(candle.close)
...
class BreakoutStrategy(Strategy):
    ...
    def _check_signal(self) -> int: #checks candles behaviors
